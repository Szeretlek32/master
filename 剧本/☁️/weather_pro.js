...
let config = {
    ...
    aqicn_api: "78eb954e87841ab2a9b853d2a7634453d7c9d599", //从http://aqicn.org/data-platform/token/#/ 上申请key填入即可
    huweather_apiKey: "9b8b19ae9fae4e82b22aca5536fe84dd", //和风天气APIkey,可自行前往 https://dev.heweather.com/ 进行获取(注意key类型选WebApi)
    lat_lon: "37.00385831456814,114.9624022753215", //请填写经纬度,直接从谷歌地图中获取即可
...
当前温度$[currentTemperature]℃ 🌡体感温度 $[apparentTemperatureMin] ~ $[apparentTemperatureMax]°C 

🍃全天风速$[windSpeed]km/h 💨当前风速$[currentWindSpeed]km/h 

全天风力等级$[windPower] 当前风力等级$[currentWindPower]

全天风向$[windDir] 当前风向$[currentWindDir]  

全天能见度$[visibility]km 当前能见度$[currentVisibility]km

🌞紫外线指数 $[uv]($[uvDesc]) 

全天相对湿度$[humidity]% 💧湿度$[currentHumidity]% 

🌅日出$[sunrise] 🌄日落$[sunset]
 
💨气压$[atmosphere]pa 当前大气压$[currentAtmosphere]pa

[48小时预报]
 
$[hourly($[month]/$[day] $[hour]:00 紫外线指数$[uv]($[uvDesc]) $[temperature]℃ $[weatherIcon]$[weather] 🌧概率$[precipProbability]%)]
